...
#ifndef RINGBUFFERDRAWING_H
#define RINGBUFFERDRAWING_H
...
struct sLineCoordinates {
    uint16_t coordinates[4];
};
typedef struct sLineCoordinates LineCoordinates_t;
#if 0
#endif
    
struct sDrawingInstr {
    LineCoordinates_t lines[20];
    TFT_colors_t color;
    uint8_t numLines;
};
typedef struct sDrawingInstr DrawingInstr_t;
    
typedef DrawingInstr_t RB_Draw_data_t;

typedef struct {
    RB_Draw_data_t   m_data[RB_DRAWING_SIZE];
