...
RC_t RB_EV_clear(RB_EV_t * const me)
{
    ...
    for (uint16_t i = 0; i < RB_EV_SIZE; i++)
    {
        me->m_data[i] = -1;
    }
...
{
    if (me->m_fillLevel < RB_EV_SIZE)
    {
        me->m_data[me->m_writeIndex++] = *data;
        
        me->m_writeIndex %= RB_EV_SIZE;
        me->m_fillLevel++;
...
uint16_t RB_EV_getCapacity(RB_EV_t * const me)
{
    ...
    return RB_EV_SIZE - me->m_fillLevel;
}
