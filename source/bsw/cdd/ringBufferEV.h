...
#ifndef RINGBUFFEREV_H
#define RINGBUFFEREV_H
...
*/
/**
 * @brief Enumeration representing different events.
 */
typedef enum {
    ev_NONE = -1,        /**< No event. */
    ev_parseJSON = 0,    /**< Event to parse JSON. */
    ev_drawOnTFT = 1,    /**< Event to draw on TFT. */
    ev_uartError = 2     /**< UART error event. */
} eEvent_t;
...
#define RB_EV_SIZE 200          /**< Size of the event buffer. */
typedef eEvent_t RB_EV_data_t; /**< Type definition for event buffer data. */
...
*/
typedef struct {
    RB_EV_data_t   m_data[RB_EV_SIZE];
 ...
 * @param RB_EV_t  * const me : IN/OUT - pointer to ringbuffer object 
 * @return RC_SUCCESS or error code
 */
RC_t RB_EV_init(RB_EV_t * const me);
 ...
 * @return RC_SUCCESS or error code
 */
RC_t RB_EV_clear(RB_EV_t * const me);
 ...
 * @return RC_SUCCESS or error code
 */
RC_t RB_EV_write(RB_EV_t * const me, RB_EV_data_t const * const data);
 ...
 * @return RC_SUCCESS or error code
 */
RC_t RB_EV_read(RB_EV_t * const me, RB_EV_data_t * const data);
 ...
 */
uint16_t RB_EV_getCapacity(RB_EV_t * const me);
 ...
 */
uint16_t RB_EV_getNumberOfStoredElements(RB_EV_t * const me);
